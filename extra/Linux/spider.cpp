...
namespace spider
{
    ...
    static void print_title()
    {
        ...
        std::printf("         -+: ++-  -*-        _// _//_//     _// _// _//  _////   _///     \n");
        std::printf("        :*-  :*-   .:.              _//                                   \n");
        std::printf("        =-    -:                 Linux Ver: x.xx  Author: Shuichiro Endo  \n");
        std::printf("\n");
    }
    ...
    static void usage(char *filename)
    {
        ...
        std::printf("        : [-T forward_proxy_type(http:h http-sslbump:hs https:s)]\n");
        std::printf("        : [-A forward_proxy_authentication(no:0 basic:1 digest:2 ntlmv2:3 spnego(kerberos):4)]\n");
        std::printf("        : [-I forward_proxy_ip] [-P forward_proxy_port]\n");
        std::printf("        : [-U forward_proxy_username] [-O forward_proxy_password] [-N forward_proxy_nthash_hexstring]\n");
        std::printf("        : [-D forward_proxy_domainname] [-W forward_proxy_workstationname]\n");
        std::printf("        : [-S forward_proxy_service_principal_name]\n");
        ...
        std::printf("        : %s -d -i 192.168.0.25 -p 1025 -m h -t 300 -T h -A 0 -I 192.168.0.50 -P 3128\n", filename);
        std::printf("        : %s -d -i 192.168.0.25 -p 1025 -m s -t 300 -T hs -A 0 -I 192.168.0.50 -P 13128\n", filename);
        std::printf("        : %s -d -i 192.168.0.25 -p 1025 -m s -t 300 -T s -A 0 -I 192.168.0.50 -P 3129\n", filename);
        std::printf("        : %s -d -i 192.168.0.25 -p 1025 -m h -t 300 -T h -A 1 -I 192.168.0.50 -P 3128 -U test01 -O p@ssw0rd\n", filename);
        std::printf("        : %s -d -i 192.168.0.25 -p 1025 -m h -t 300 -T h -A 2 -I 192.168.0.50 -P 3128 -U test01 -O p@ssw0rd\n", filename);
        std::printf("        : %s -d -i 192.168.0.25 -p 1025 -m h -t 300 -T h -A 3 -I 192.168.0.50 -P 3128 -U test01 -O p@ssw0rd -D test.local -W WORKSTATION\n", filename);
        std::printf("        : %s -d -i 192.168.0.25 -p 1025 -m h -t 300 -T h -A 3 -I 192.168.0.50 -P 3128 -U test01 -N de26cce0356891a4a020e7c4957afc72 -D test.local -W WORKSTATION\n", filename);
        std::printf("        : %s -d -i 192.168.0.25 -p 1025 -m h -t 300 -T h -A 4 -I 192.168.0.50 -P 3128 -S HTTP/squid.test.local@TEST.LOCAL\n", filename);
...
{
    int opt;
    const char *optstring = "h:4:6:u:l:f:di:p:m:t:T:A:I:P:U:O:N:D:W:S:r:e:k:v:s";
    opterr = 0;
    ...
    while((opt = getopt(argc, argv, optstring)) != -1)
    {
        switch(opt)
        {
            ...
            case 'T':
                forward_proxy_type = optarg;
                break;

            case 'A':
                forward_proxy_authentication = atoi(optarg);
                break;
            ...
            case 'D':
                forward_proxy_domainname = optarg;
                break;
            ...
            case 'S':
                forward_proxy_service_principal_name = optarg;
                break;
    ...
    legacy = OSSL_PROVIDER_load(NULL,
                                "legacy");
    if(legacy == NULL)
    {
#ifdef _DEBUG
        std::printf("[-] OSSL_PROVIDER_load error: legacy\n");
#endif
        exit(-1);
    }

    deflt = OSSL_PROVIDER_load(NULL,
                               "default");
    if(deflt == NULL)
    {
#ifdef _DEBUG
        std::printf("[-] OSSL_PROVIDER_load error: default\n");
#endif
        exit(-1);
    }
       ...
       && !pipe_destination_port.empty())
    {
        ...
        if(forward_proxy_authentication < 0 ||
           forward_proxy_authentication > 4)
        {
