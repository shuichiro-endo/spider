/*
 * Title:  spider ntlmv2.cpp (Windows)
...
namespace spider
{
    ...
    int Ntlmv2::rand_bytes(unsigned char *buffer,
...
error:
                              ...
                              int32_t output_size)
    {
        ...
        ret_b = CryptStringToBinaryA((const char *)input,
                                     0,
                                     CRYPT_STRING_BASE64,
                                     NULL,
                                     &required_size,
        ...
        {
#ifdef DEBUGPRINT
            std::printf("[-] decode_base64 CryptStringToBinaryA error\n");
#endif
            goto error;
        }
        ...
        {
#ifdef DEBUGPRINT
            std::printf("[-] decode_base64 required_size error: %d\n",
                        required_size);
#endif
            goto error;
        }
...
error:
                             ...
                             int output_size)
    {
        ...
        {
#ifdef DEBUGPRINT
            std::printf("[-] get_md4_hash CryptAcquireContext error\n");
#endif
            goto error;
        }
        ...
        {
#ifdef DEBUGPRINT
            std::printf("[-] get_md4_hash CryptCreateHash error\n");
#endif
            goto error;
        }
        ...
        {
#ifdef DEBUGPRINT
            std::printf("[-] get_md4_hash CryptHashData error\n");
#endif
            goto error;
        }
        ...
        {
#ifdef DEBUGPRINT
            std::printf("[-] get_md4_hash CryptGetHashParam error\n");
#endif
            goto error;
        }
        ...
        {
#ifdef DEBUGPRINT
            std::printf("[-] get_md4_hash output_size error required_size: %d output_size: %d\n",
                        required_size,
                        output_size);
#endif
            goto error;
...
error:
    ...
    int Ntlmv2::get_md5_hash(const unsigned char *input,
                             int input_length,
                             unsigned char *output,
                             int output_size)
    {
        ...
        {
#ifdef DEBUGPRINT
            std::printf("[-] get_md5_hash output_size error required_size: %d output_size: %d\n",
                        required_size,
                        output_size);
#endif
            goto error;
...
error:
                             ...
                             int output_size)
    {
        ...
        int key2_length = 0;
        unsigned char *key2 = (unsigned char *)calloc(64,
                                                      sizeof(char));

        int ipad_key_length = 64 + input_length;
        unsigned char *ipad_key = (unsigned char *)calloc(ipad_key_length,
                                                          sizeof(char));
        int ipad_key_md5_length = 16;
        unsigned char *ipad_key_md5 = (unsigned char *)calloc(ipad_key_md5_length,
                                                              sizeof(char));
        int opad_key_length = 64 + 16;
        unsigned char *opad_key = (unsigned char *)calloc(opad_key_length,
                                                          sizeof(char));


        if(key_length > 64)
        {
            ret = get_md5_hash(key,
                               key_length,
                               key2,
                               64);
            if(ret < 0)
            {
#ifdef DEBUGPRINT
                std::printf("[-] get_hmac_md5 get_md5_hash error\n");
#endif
                goto error;
            }
        ...
        for(int i=0; i<64; i++)
        {
            ipad_key[i] = ipad_key[i] ^ 0x36;
        }

        std::memcpy(ipad_key + 64,
                    input,
                    input_length);

        ret = get_md5_hash(ipad_key,
                           ipad_key_length,
                           ipad_key_md5,
                           ipad_key_md5_length);
        if(ret < 0)
        ...
        for(int i=0; i<64; i++)
        {
            opad_key[i] = opad_key[i] ^ 0x5c;
        }

        std::memcpy(opad_key + 64,
                    ipad_key_md5,
                    ipad_key_md5_length);

        ret = get_md5_hash(opad_key,
                           opad_key_length,
                           output,
                           output_size);
        if(ret < 0)
...
error:
                                      ...
                                      size_t output_size)
    {
        ...
        {
#ifdef DEBUGPRINT
            printf("[-] convert_utf8_to_utf16 MultiByteToWideChar error: %d\n",
                   GetLastError());
#endif
            return -1;
        }
...
//          print_bytes(server_challenge, 8);
            ...
            {
#ifdef DEBUGPRINT
                std::printf("[-] generate_response_ntlmv2 client_challenge generate error\n");
#endif
                goto error;
            }
