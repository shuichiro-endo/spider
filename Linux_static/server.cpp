/*
 * Title:  spider server.cpp (Linux static)
        ...
        while(1)
        {
            ...
            if(ret > 0)
            {
                ...
                if(rec <= 0)
                {
                    ...
                    {
#ifdef _DEBUG
                        std::printf("[-] [server <- target] forwarder_send_data recv error: %d\n",
                                    errno);
#endif
                        break;
                    }
    ...
    int32_t Server::do_socks5_connection(std::shared_ptr<Socks5message> socks5_message)
    {
        ...
        }else if(socks_request->atyp == 0x3)    // domain name
        {
            ...
            if(domainname_string.find(":") == std::string::npos)   // ipv4 address or domainname
            {
                ...
                ret = cares_manager->get_addr_info(domainname_string.c_str(),
                                                   std::to_string(ntohs(target_addr.sin_port)).c_str(),
                                                   AF_INET,
                                                   &target_addr,
                                                   NULL);
                if(ret != 0)
                {
                    ...
                    ret = cares_manager->get_addr_info(domainname_string.c_str(),
                                                       std::to_string(ntohs(target_addr6.sin6_port)).c_str(),
                                                       AF_INET6,
                                                       NULL,
                                                       &target_addr6);
                    if(ret != 0)
